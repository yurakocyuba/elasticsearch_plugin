...
bulker_pool::bulker_pool(
   size_t size, size_t bulk_size,
   const std::vector<std::string> url_list,
   const fc::path& dir, bool dump): pool_size(size), bulk_size(bulk_size)
{
   for (int i = 0; i < pool_size; ++i) {
      auto dump_path = dir / ("bulk-dump-" + std::to_string(i));
      bulkers.emplace_back( new bulker(bulk_size, url_list, dump_path, dump) );
   }
}
